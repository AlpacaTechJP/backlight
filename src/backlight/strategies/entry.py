...
from backlight.trades.trades import Transaction, Trades, make_trades, make_trade
from backlight.strategies.common import Action


def _entry(amount: float, idx: pd.Timestamp, symbol: str) -> pd.Series:
...
def direction_based_entry(
    ...
    for direction, action in direction_action_dict.items():
        ...
        trades += [_entry(amount, idx, df.symbol) for idx in target_index]
