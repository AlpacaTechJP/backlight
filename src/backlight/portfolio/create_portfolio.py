...
def create_simple_portfolio(
    mkt: List[MarketData], sig: List[Signal], strategy_name: str, strategy_params: dict, principal: float
):
    """
    ...
    trades = Parallel(n_jobs=-1)(
        [delayed(strategy)(market, asset, **strategy_params) for (asset,market) in zip(sig,mkt)]
    )

    # Construct positions and return Portfolio
    positions = Parallel(n_jobs=-1)(
        [delayed(calc_positions)(trade, market, principal = principal_per_asset) for (trade,market) in zip(trades,mkt)]
    )
